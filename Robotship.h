...
//TO-DO: -add: Robotshipship dash background movement animation
//note: start at 0
...
//TO-DO: -update: this to use "Robotship"
...
*/
    ...
    void load_pngPrev(char *filename, unsigned int glITextureObject);
    void load_png(char *filename, GLuint glITextureObject);
...
*/
	...
	//added by Mike, 20201130
	// methods to draw the parts of the Robotship
...
//    Pilot(float xPos, float yPos, float zPos,int windowWidth,int windowHeight);
    Robotship(float xPos, float yPos, float zPos, float fWindowWidth, float fWindowHeight);
...
//    	drawPilot();
	...
	// draws the entire Robotship
	//void drawRobotship(float xPos, float yPos, float zPos);
    void drawPilot();
//    void drawValkyrie();
