...
//TO-DO: -update: this
class Level3D: public MyDynamicObject
{
private:
    int currentState;
...
//		void drawLevel(GLfloat x, GLfloat y, GLfloat z, char *string);	
    ...
    bool isLevel2DCollideWith(MyDynamicObject* mdo);
		...
		//added by Mike, 20210925
		bool isLevel3DCollideWith(MyDynamicObject* mdo);
...
//    virtual void openGLDrawTexture(float x, float y, float textw, float texth);
    ...
    void openGLDrawTexture(float x, float y, float z, float textw, float texth, std::string sTileId);
    void openGLDrawTextureQuadOKNotYetCube(float x, float y, float textw, float texth, std::string sTileId);
